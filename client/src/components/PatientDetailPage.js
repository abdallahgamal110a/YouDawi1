    ...
    return (
        <div className="patient-detail">
            ...
            <div className="patient-info flex items-center">
                ...
                />
                <div>
                    <h2 className="text-3xl font-semibold">{`${patient.firstName} ${patient.lastName}`}</h2>
            ...
            <h3 className="text-3xl font-bold mb-2">Health History</h3>
                ...
                {patient.healthHistory.map((historyItem) => (
                    <div key={historyItem._id} className="history-item">
                        {historyItem.familyStatus && (
                            <p className="text-lg"><strong>Family Status:</strong> {historyItem.familyStatus}</p>
                        )}
                        {historyItem.diseases && (
                            <p className="text-lg"><strong>Diseases:</strong> {historyItem.diseases}</p>
                        )}
                        {historyItem.examinations && (
                            <p className="text-lg"><strong>Examinations:</strong> {historyItem.examinations}</p>
                        )}
                        {historyItem.diagnosis && (
                            <p className="text-lg"><strong>Diagnosis:</strong> {historyItem.diagnosis}</p>
                        )}
                        {historyItem.treatment && (
                            <p className="text-lg"><strong>Treatment:</strong> {historyItem.treatment}</p>
                        )}
                            ...
                            <div className="prescriptions mt-4">
                                ...
                                {historyItem.prescriptions.map((prescription) => (
                                    <div key={prescription.prescriptionId} className="prescription p-2 bg-white border border-gray-300 rounded mt-2">
                                        <p className="text-lg"><strong>Doctor:</strong> {prescription.doctorName}</p>
                                        <p className="text-lg"><strong>Date Issued:</strong> {new Date(prescription.dateIssued).toLocaleDateString()}</p>
                                        <p className="text-lg"><strong>Instructions:</strong> {prescription.instructions}</p>
                                        <p className="text-lg"><strong>Medications:</strong></p>
                                        <ul className="list-disc ml-5">
                                            {prescription.medications.map((med) => (
                                                <li key={med.name} className="text-lg">
